...
class WRFDiagnosticsAccessor:
    ...
    def _get_var(self, names, mandatory=True):
        ...
        for name in names:
            ...
            if name == 'P' and 'pres' in self._obj:
                return self._obj['pres']
            if name == 'T' and 'theta' in self._obj:
                return self._obj['theta']
    ...
    def _wrap_result(self, data, ref_var, name, units, description):
        ...
        # 2. 维度减少了1维 (通常是垂直层)
            ...
            if len(final_dims) != data.ndim:
                if len(dims) == 4:
                    final_dims = [dims[0], dims[2], dims[3]]
                elif len(dims) == 3:
                    final_dims = [dims[1], dims[2]]
        else:
    ...
    def dbz(self, use_varint=True, use_liqskin=False):
        ...
        return self._wrap_result(
            res, self._get_var(['T', 'theta']), 'dbz', 'dBZ', 'Radar Reflectivity'
        )
    ...
    def eth(self):
        ...
        return self._wrap_result(
            res, self._get_var(['T', 'theta']), 'eth', 'K', 'Equivalent Potential Temperature'
        )
    ...
    def td(self):
        ...
        return self._wrap_result(
            res, self._get_var(['T', 'theta']), 'td', 'degC', 'Dewpoint Temperature'
        )
    ...
    def pw(self):
        ...
        return self._wrap_result(
            res, self._get_var(['T', 'theta']), 'pw', 'kg m-2', 'Precipitable Water'
        )
    ...
    def cape_2d(self):
        ...
        res = wrf.cape_2d(
            full_p * 0.01, tk, qv, height, HGT.values, PSFC.values * 0.01, ter_follow=True
        )
        ...
        def wrap(d, n, u, desc):
            return self._wrap_result(d, ref, n, u, desc)
    ...
    def cloudfrac(self):
        ...
        res = wrf.cloudfrac(
            full_p, relh, vert_inc_w_height=0, low_thresh=97000, mid_thresh=80000, high_thresh=45000
        )
        ...
        def wrap(d, n, desc):
            return self._wrap_result(d, ref, n, '%', desc)
    ...
    def avo(self):
        ...
        try:
            ...
            raise KeyError(
                "Calculation of 'avo' requires MAPFAC_U and MAPFAC_V. Please ensure these variables are in your dataset."
            )
        ...
        # 3. 实在找不到，直接报错，强迫用户检查文件
            ...
            raise KeyError(
                "Global attributes 'DX' and 'DY' (grid spacing) are missing from the Dataset. "
